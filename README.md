...
`collection`), or the hover **Move…** control on a document card. Rename or delete a collection from
the sidebar (delete reassigns its documents to `General`):

- `PUT /api/documents/collections/{name}` (form `new_name`) — rename
- `DELETE /api/documents/collections/{name}` — delete (reassign docs to `General`)
